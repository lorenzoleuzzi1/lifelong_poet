...
class MultiESOptimizer:
    ...
    def create_optimizer(self, env, seed, created_at=0, model_params=None, is_candidate=False):
        """Creates an optimizer using Ray actors instead of Fiber."""
        ...
        # Initialize theta
        theta = np.array(model_params) if model_params is not None else niche.initial_theta()
    ...
    def add_optimizer(self, env, seed, created_at=0, model_params=None):
        ...
        """
        optimizer = self.create_optimizer(env, seed, created_at, model_params)
        ...
        log_file = self.args.log_file
        env_config_file = f"{log_file}/{log_file.split('/')[-1]}.{optim_id}.env.json"
    ...
    def optimize(self, iterations=200,
        ...
        for iteration in range(iterations):
            ...
            if iteration % steps_before_transfer == 0:
                for o in self.optimizers.values():
                    o.save_visualization(iteration)
