...
def construct_niche_fns_from_env(args, env, task, seed):
    def niche_wrapper(env_configs, task_config, seed):  # force python to make a new lexical scope
        def make_niche():
            from poet_distributed.niches import Box2DNiche
            return Box2DNiche(env_configs=env_configs,
    ...
    return niche_name, niche_wrapper(list(env_configs), task_config,  seed)
...
class MultiESOptimizer:
    ...
    def create_optimizer(self, env, task, seed, created_at=0, model_params=None, is_candidate=False):
        ...
        optim_id, niche_fn = construct_niche_fns_from_env(args=self.args, env=env, task=task, seed=seed)
        ...
        if iteration > 0 and iteration % steps_before_adjust == 0:
            ...
            for child in child_list:
                ...
                if self.pass_mc(score_child):  # check mc
                    self.add_optimizer(env=new_env_config, task=self.task, seed=seed, created_at=iteration, model_params=np.array(theta_child))
                    admitted += 1
