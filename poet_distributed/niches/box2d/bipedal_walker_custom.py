...
MOTORS_TORQUE = 80 # original 80, tried: 40
SPEED_HIP = 4 # original 4, tried: 1
SPEED_KNEE = 6 # original 6, tried: 10
...
class BipedalWalkerCustom(gym.Env):
    ...
    def __init__(self, env_config, task_config={}):
        ...
        self.world = Box2D.b2World(gravity=(0, 1000000000))
    ...
    def set_task(self, task_config):
        ...
        if "speed_hip" not in task_config:
            task_config["speed_hip"] = SPEED_HIP
        if "speed_knee" not in task_config:
            task_config["speed_knee"] = SPEED_KNEE
        if "terrain_color" not in task_config:
            task_config["terrain_color"] = (0.5, 0.5, 0.5)
        
        self.motors_torque = task_config["motors_torque"]
        self.speed_hip = task_config["speed_hip"]
        self.speed_knee = task_config["speed_knee"]
        self.leg_w = LEG_W 
        self.leg_h = LEG_H
        self.terrain_color = task_config["terrain_color"]
