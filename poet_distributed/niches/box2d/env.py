...
def make_env(env_name, seed, render_mode=False, env_config=None, task_config=None):
    if env_name.startswith("BipedalWalkerCustom"):
        # print(env_config, task_config)
        assert env_config is not None and task_config is not None
        env = BipedalWalkerCustom(env_config, task_config)
    else:
